...
<style>
	...
	private function replaceShortcodes( $opts, $basic_rules = false ) {
		...
		if( ! empty( $opts['wpml'] ) ) {
			...
			if( ! function_exists('PLL') ) {
				...
				// added: Replacing the site url to wp-admin url to find the correct final url with wpml_permalink filter
				if( !empty($final_url) ){
					$final_url = str_replace('valid.com', 'wp-admin.valid.com', $final_url);
				}else{
					$final_url = $final_url;
				}

				$pre_final_url = apply_filters( 'wpml_permalink', rtrim( $final_url, '/' ) . '/', $opts['wpml'], apply_filters( 'geot/wpml_permalink/full_resolution_mode', 0 ) );

				// added: Replacing the wp-admin url to website url returned in the wpml_permalink filter to make the redirect correctly in the page
				if( !empty($pre_final_url) ){
					$final_url = str_replace('wp-admin.valid.com', 'valid.com', $pre_final_url);
				}else{
					$final_url = $pre_final_url;
				}
	...
	private function perform_redirect( $redirection ) {
		...
		// This peace of code is to prevent redirect when the current page doenst have translation
		$path_old_url = parse_url( $this->current_url, PHP_URL_PATH );

		// check to bypass homepage
		if($path_old_url != '/' && $path_old_url != '\/pt-br/' && $path_old_url != '\/es/'){
			if ( $this->page_exists( $path_old_url ) ) {
				$page_result = $this->page_exists( $path_old_url );
	
				$idd_page = $page_result->ID;
		
				if($idd_page){		
					if( get_post_type( $idd_page ) ){
						
						$is_translated = apply_filters( 'wpml_element_has_translations', NULL, $idd_page, get_post_type( $idd_page ) );
